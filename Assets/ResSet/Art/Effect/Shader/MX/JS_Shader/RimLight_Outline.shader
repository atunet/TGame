Shader "Mx_JS/RimLight_Outline" {
	Properties {
		_Color ("Main Color", Color) = (1,1,1,1)
		_OutlineColor ("Outline Color", Color) = (0,0,0,1)
		_OutlineWidth ("Outline Width", Range(0.0,0.5)) = 0.0//0.035
	...
	//	};
		...
		float4 _clip(float4 vert)
		{
            ...
            float3 v = vert - (invZdotN * (pd + dot(vert, pnrm))) * z+float3(0,0.1f,0);
			return float4(v,1);
		}
		v2fs vert( appdata_base v )
		{
			...
			float4 cv = mul(_Object2World,v.vertex);
			
			float4 vv  = mul(_World2Object,_clip(cv)); 

			o.pos = mul(UNITY_MATRIX_MVP, vv);
