...
Shader "Mx/UVscroll2_Mask+BLEND" {
Properties {
	_MainTex ("1st layer (RGB)", 2D) = "white" {}
	_alphaTex ("Alpha Layer",2D)="white"{}
	_DetailTex ("2nd layer (RGB)", 2D) = "white" {}
	_ScrollX ("1st layer Scroll speed X", Float) = 1.0
	_ScrollY ("1st layer Scroll speed Y", Float) = 0.0
	_Scroll2X ("Alpha Layer Scroll speed X", Float) = 1.0
...
SubShader {
	...
	v2f vert (appdata_full v)
	{
		...
		o.uv.xy*=_alphaTex_ST.xy;
		o.uv.xy+=_alphaTex_ST.zw;
	...
	Pass {
		...
		fixed4 frag (v2f i) : COLOR
		{
			...
			fixed4 tex1 = tex2D(_alphaTex,i.uv.xy);
