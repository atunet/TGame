...
public class MyFollow : MonoBehaviour 
{
	...
	public bool Follow
	{
		...
		set {
            ...
            if(m_follow)
            {
                ...
                Utility.Log("MyFollow:xdist:"+m_xDistance+",ydist:"+m_yDistance);
            }
        }
	}
	...
	void Update () 
	{
		...
		if (m_target == null) 
		{
			Utility.LogWarning("Follow target is null");
			return;
		}
       ...
       // //float targetY = Mathf.Lerp(transform.position.y, m_Player.position.y, ySmooth*Time.deltaTime);
        ...
        //Utility.Log("MyFollow,now pos:x:"+transform.position.x+",y:"+transform.position.y+",z:"+transform.position.z);
	}
}
